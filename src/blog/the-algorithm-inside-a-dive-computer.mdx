...
cover:
  src: "../assets/images/blog/the-algorithms-inside-a-dive-computer/cover.webp"
  alt: "A diver suspended in open blue water, exhaled bubbles rising to the surface"
  caption: "A diver in open water, using a computer on their wrist to track decompression obligation."
---
...
>
> Given everything I have breathed so far, how shallow does the model permit me to be right now, and how long must I wait before I can go shallower?
...
We'll model 16 tissue compartments using the nitrogen half-times from the ZH-L16C table in Subsurface[^subsurface].
...
The `HALF_TIMES` array from the previous section has been absorbed into `ZH_L16C_N2.half_times`. We've expanded the model to include `a` and `b` arrays from Subsurface's published C++ implementation[^subsurface]. These coefficients are for nitrogen; helium has its own coefficients and half-times. Bühlmann produced three sets of a/b coefficients for nitrogen: A, B, and C. The choice of dataset affects how conservative the resulting model is.
...
With Haldane's equation to track nitrogen loading and M-values to define a model ceiling, the algorithm itself is mostly a loop. At each time step, for example once per second, it does the same three things:
...
If the ceiling depth is zero, no compartment currently requires the diver to remain below the surface under this model. A positive ceiling sets a depth the diver must not ascend above while it remains in effect. It does not necessarily imply a future stop at that exact depth: gas exchange during ascent may lift the ceiling before the diver reaches it.

Many dive computers also display a recommended safety stop, commonly three minutes at five metres, even when no decompression stop is required. This provides an additional margin. The model estimates tissue loading but cannot capture every physiological factor affecting decompression risk[^gradientfactors]. A safety stop is distinct from a required decompression stop, which is needed when continued ascent would violate the model's limits.
...
We can also watch the nitrogen tensions behind this profile. Move the slider below to see all sixteen compartments at that moment. The arrows show whether each compartment is taking up or releasing nitrogen. A compartment can be off-gassing while its tension is still below ambient pressure.
...
All seven assertions pass. The dive takes 31 minutes 45 seconds, including descent, ascent and the three-minute safety stop. The safety stop is part of our planned profile; the model does not request it as a decompression stop. This result applies to the air-only ZH-L16C model above, starting from surface equilibrium without gradient factors. It is an educational model check, not a guarantee against decompression sickness.
